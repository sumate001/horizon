...
## ตัวตรวจจับ — สองเรื่องที่ต้องรู้ก่อนอ่านตัวเลข

**Weak signal: ประตูคือ "กำลังโต" ไม่ใช่ "แปลก"** วัดจาก 994 candidates จริง — 987
ตัวเป็นข่าวเดี่ยว ซึ่ง Kleinberg มีข้อมูลวันเดียว burst จึงเป็น 0 เชิงโครงสร้าง
เหลือ novelty กับ isolation ที่วัด "ไม่เหมือนคลังข่าว" ทั้งคู่ และ**ข่าวเดี่ยวที่ไม่
เกี่ยวอะไรเลยทำคะแนนนั้นได้สูงสุดโดยการไม่เกี่ยวข้อง** อันดับต้น ๆ จึงเป็นข่าวเบา

`Candidate.is_emerging` (burst > 0) จึงเป็น**ประตู**ก่อนดูคะแนน ส่วน
`WEAK_SIGNAL_T` ลดจาก 0.65 เป็น 0.20 เพราะ 0.65 ไปไม่ถึงเชิงโครงสร้าง — ตัวที่ดี
ที่สุดในคลังทั้งหมดได้ 0.364 บั๊กคลาสเดียวกับ `temporal_weight` คือค่าคงที่ที่
ตั้งเทียบกับช่วงทฤษฎีที่ข้อมูลจริงไม่เคยกิน

**Trend breakout: เงียบเพราะยังไม่ถึงเวลา ไม่ใช่พัง** `PROVISIONAL_DAYS = 14` —
คลัสเตอร์ที่ประวัติไม่ถึงจะไม่ประกาศ breakout เพราะ z-score บนหน้าต่างไม่กี่บาน
บอกเรื่องขนาดตัวอย่างมากกว่าเรื่องโลกจริง `/api/v1/stats` จึงคืน
`detection.trend_ready_at` มาด้วย — วันที่เครื่องนี้จะรายงาน breakout ได้ครั้งแรก
**จำนวน 0 กำกวม** "ไม่มีอะไรผิดปกติ" กับ "ยังตรวจไม่ได้" หน้าตาเหมือนกันจากข้างนอก
...
`off_topic` **ไม่ใช่ระดับที่สี่ของความผิด** — มันบอกว่าตรวจจับถูกแล้ว เรื่องนั้นแค่
ไม่ใช่ประเด็นที่กองบรรณาธิการนั้นติดตาม เป็นข้อมูลป้อนกลับเรื่อง*ความเกี่ยวข้อง*
ไม่ใช่*ความถูกต้อง* จึงไม่อยู่ใน `ACCURACY_VERDICTS` และไม่เลื่อนสถานะไปทางใดทางหนึ่ง
เดิม DESK ส่ง `false_signal` ทุกครั้งที่ตีกลับ ซึ่งแปลว่าบรรณาธิการที่เคลียร์ข่าว
นอกประเด็นกำลังสอนเรดาร์ให้กดการตรวจจับที่ทำงานถูกอยู่แล้ว และไม่มีอะไรในระบบไหน
แสดงให้เห็นเลย

**`top_events[].location`** อยู่ในสัญญาแล้ว Horizon สกัดค่านี้ให้ 4 ใน 5 เหตุการณ์
มาตลอดแต่ไม่เคยส่งไปไหน เพราะไม่ได้อยู่ใน schema — DESK เอาไปแปลงเป็นพิกัดและ
ปักแผนที่ ไม่ใช่ฟิลด์บังคับ: สัญญาณที่ส่งไปก่อนหน้านี้ยังถูกต้อง และอีก 20%
ไม่ได้ระบุสถานที่จริง ๆ
